...
server.post('/api/update_store',(req,res)=>{
    ...
    con.query(sql,[name,fei,qisong,info,message,pos,uid],(err,result)=>{
...
server.post('/api/set_c_address',(req,res)=>{
    ...
    con.query(sql,[uid,name,phone,address,gender,label],(err,result)=>{
        if(!err){
            if(result.insertId){
                res.json({code:2,message:'ok',data:result.insertId});
            }
...
server.post('/api/get_c_address',(req,res)=>{
    ...
    con.query(sql,[name,address,gender,label,phone,uid,id],(err,result)=>{
        if(!err){
            res.json({code:2,message:'ok',data:result[0]})
        }
    })
...
server.get('/api/get_order',(req,res)=>{
   let sql='select * from order_info where uid=?';
   con.query(sql,[req.query.uid],(err,r)=>{
       if(!err){
           ...
           let sql=
               'select  * from choice_goods where oid in'+s.slice(0,-1)+');'
           con.query(sql,(err,result)=>{
               if(!err){
                   res.json({
                       code:2,
